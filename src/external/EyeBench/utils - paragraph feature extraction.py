# Vendored from the lab's EyeBench / OneStop pipeline -- kept as received so a
# newer copy can replace it wholesale. The `src.configs` imports below resolve to
# `src/external/EyeBench/configs/` (aliased by `paragraph_trial_features.py`,
# which is also how you should load this file). Only local change: the f-string
# on line ~296 was reflowed onto one line, since multi-line f-string expressions
# need Python 3.12 and this env is 3.11.
import warnings
...
def add_missing_categories_and_flatten(
    ...
    if len(groupby_fields) < len(new_index):
        # NOTE: kept on one line -- multi-line expressions inside f-strings (PEP
        # 701) need Python 3.12, and this project's env is 3.11.
        logger.warning(
